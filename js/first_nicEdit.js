...
buttonList : ['save','bold','italic','underline','left','center','right','justify','ol','ul','image','upload','link'],
iconList : {
    "bgcolor":1,
    "forecolor":2,
    "bold":3,
...
var nicEditors={
    ...
    allTextAreas:function(C){
        var A=document.getElementById("first_collection");
            nicEditors.editors.push(new nicEditor(C).panelInstance(A))
            return nicEditors.editors
        },
    idTextArea:function(C, area_id){
...
var nicLinkButton=nicEditorAdvancedButton.extend({
    addPane:function(){
        ...
        this.addForm({
            "":{
                type:"title",
                txt:"Добавить/Редактировать ссылку"
            },
            href:{
                type:"text",
                txt:"URL",
...
var nicImageButton=nicEditorAdvancedButton.extend({
    addPane:function(){
        ...
        this.addForm({
            "":{
                type:"title",
                txt:"Добавить/Редактировать изображение"
            },
            src:{
                type:"text",
                txt:"URL",
...
var nicUploadButton=nicEditorAdvancedButton.extend({
    ...
    addForm:function(){
        ...
        $('#magic_file').change(function(){
            $('#form_magic_file').submit();
            $('.nicEdit-pane').html('Загружаем... <img src="/images/ajax-loader.gif" />').css({width:'90px'});
                ...
                $.ajax({
                  ...
                  success: function(data){
                      if(data.status == 'success'){
                          ...
                          image.nicCommand("insertImage", '/uploads/'+data.name);
                          pan.removePane();
                      }else if(total_request >= 10){
                          clearInterval(interval);
                      }
                  }
                });
    ...
    startUpload:function(){
       ...
       // this.myStatus.setContent('<img src="/images/ajax-loader.gif" style="float: right; margin-right: 40px;" /><strong>Загружаем...</strong><br />Пожалуйста подождите');
        this.myForm.submit();
        var image = this.ne;
        var pan = this;
